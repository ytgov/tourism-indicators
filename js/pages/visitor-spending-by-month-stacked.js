...
async function generateChart() {
    const rawData = await loadCSVData('data/vw_kpi_moneris_total_spending_ytd_summary.csv?' + Math.random());
    if (!rawData.length) {
    ...
    Highcharts.chart('monthly-chart', {
        ...
        title: {
            text: 'Visitor spending By month'
        },
        xAxis: {
            ...
            formatter: function () {
                ...
                this.points.forEach(point => {
                    tooltip += point.series.name + ': $' +
                        Highcharts.numberFormat(point.y / 1000000, 1) + 'M<br/>';
                });
                return tooltip;
            }
        },
