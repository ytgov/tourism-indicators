...
async function generateChart() {
    const csvData = await loadCSVData('data/vw_kpi_cbre_revpar_ytd_summary.csv?'+Math.random());
    if (!csvData) {
    ...
    data.forEach(row => {
        ...
        const occupancyRate = Math.round(parseFloat(row[headers.indexOf('monthly_avg_revenue_per_room')]) * 10) / 10;
