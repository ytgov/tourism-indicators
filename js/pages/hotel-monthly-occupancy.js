...
async function generateChart() {
    const csvData = await loadCSVData('data/vw_kpi_cbre_occupancy_rate_ytd_summary.csv?'+Math.random());
    if (!csvData) {
    ...
    data.forEach(row => {
        ...
        const occupancyRate = Math.round(parseFloat(row[headers.indexOf('monthly_avg_occupancy_rate')]) * 10) / 10;
