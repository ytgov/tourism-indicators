...
.sort((a, b) => parseInt(a[0]) - parseInt(b[0]));
        ...
        let lastActualOrEstimateYear = null;
        let lastActualOrEstimateValue = null;
        let forecastStarted = false;

        // First pass: Identify last 'Actual' or 'Estimate' year and process data
        rows.slice(1).forEach(row => {
            ...
            if (!isNaN(year) && !isNaN(visitors)) {
                ...
                if (!forecastStarted && (type.trim() === 'Actual' || type.trim() === 'Estimate')) {
                    // Update last actual or estimate year/value
                    if (lastActualOrEstimateYear === null || year > lastActualOrEstimateYear) {
                        lastActualOrEstimateYear = year;
                        lastActualOrEstimateValue = visitors;
                    }
                }

                // Only add range for 'Forecast' type
                if (type.trim() === 'Forecast') {
                    forecastStarted = true; // Mark forecast start
                    const lowEstimate = Math.round((visitors * 0.90)/100) *100;
                    const highEstimate = Math.round((visitors * 1.1)/100) *100;
                    seriesData.range.push([year, lowEstimate, highEstimate]);
                }
            }
        });

        // Add range for the last year before forecast
        if (lastActualOrEstimateYear && lastActualOrEstimateValue) {
            const lowEstimate = lastActualOrEstimateValue * 1;
            const highEstimate = lastActualOrEstimateValue * 1;
            seriesData.range.push([lastActualOrEstimateYear, lowEstimate, highEstimate]);
        }
